...
**왜 되는지 모르면, 언제 안 될지도 모릅니다.**  
코드가 동작하는 것에 그치지 않고, 왜 동작하는지부터 확인합니다.
화면부터 서버, 인프라까지 다뤄온 경험을 바탕으로 문제가 발생한 지점을 끝까지 추적하고 원인을 검증합니다.

![검증 기반 문제해결](https://img.shields.io/badge/검증_기반_문제해결-000000?style=for-the-badge)
![AWS 인프라 운영](https://img.shields.io/badge/AWS_인프라_운영-000000?style=for-the-badge)
![인증/API 연동 설계](https://img.shields.io/badge/인증%2FAPI_연동_설계-000000?style=for-the-badge)
![풀스택 경험](https://img.shields.io/badge/풀스택_경험-000000?style=for-the-badge)

**Currently**

- 개인 사이드 프로젝트 **"내친소"**(지인 기반 소개 서비스) 기획 중
...
- 비회원 사용자도 이용 가능한 신규 서비스 전체 기획
- 매직링크/OTP 기반 비회원 인증, 기존 통합 인증서버와의 OAuth2 신규 클라이언트 연동 설계
- DB/API 설계(REST API 42개), 배포 전 데이터 정합성 결함 다수 사전 발견
...
- 펌웨어 서명 인프라가 없던 상태에서 처음부터 구축
- OpenSSL로 ECDSA P-256 키 생성, `codeSigning` 전용 X.509 인증서 발급 후 AWS Certificate Manager 등록
